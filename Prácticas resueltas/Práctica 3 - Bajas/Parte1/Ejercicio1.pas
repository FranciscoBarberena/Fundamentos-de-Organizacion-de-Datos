...
Begin
  ...
  close(textoSinDni);
End;
Procedure realizarBajaFisica(Var arch : archivoEmp);

Var numAEliminar: integer;
  e: empleado;
  encontro: boolean;
  posAEscribir,posUltimo: longInt;
Begin
  reset(arch);
  encontro := false;
  write('Ingrese el número del empleado que quiere borrar: ');
  readln(numAEliminar);
  While ((Not eof(arch)) And (Not encontro)) Do
    Begin
      read(arch,e);
      If (e.numero = numAEliminar) Then
        Begin
          encontro := true;
          posAEscribir := filePos(arch) -1;
          posUltimo := fileSize(arch)-1;
          seek(arch, posUltimo);
          Read(arch,e);
          seek(arch,posAEscribir);
          write(arch,e);
          seek(arch,posUltimo);
          truncate(arch);
        End;
...
Begin
  ...
  writeln('Seleccione la operacion deseada:',#13#10,'1: Buscar un empleado.',#13#10,'2: Listar todos los empleados.',#13#10,'3: Listar todos los empleados mayores de 70 anios.',
          #13#10,'4: Agregar empleados.',#13#10,'5: Modficar una edad.',#13#10,'6: Exportar a archivo de texto.',#13#10,'7: Exportar a archivo de texto aquellos empleados sin DNI.',#13#10,
          '8. Realizar baja fisica por numero de empleado.',#13#10);
    ...
    Begin
      Case opcionElegida Of 
        ...
        8:
           Begin
             opcionValida := true;
             realizarBajaFisica(archivo);
           End;
        Else
          Begin
            writeln('Ingrese un numero valido (entre 1 y 8).');
            readln(opcionElegida);
          End;
      End;
